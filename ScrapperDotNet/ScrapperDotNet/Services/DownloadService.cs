...
namespace ScrapperDotNet.Services
{
    public class DownloadService(ILogger<DownloadService> logger, ScrapperContext context, IBrowserRunnerAsync browser, AppSettings settings, IAddressService addressService, IAiClient aiClient, IPageParser pageParser, IFileService fileService, IFtpDownloader ftpDownloader) : IDownloadService
    {
        private const string PageInsertCommand = "EXEC InsertPage  @addressId, @content, @contentType, @pageId OUTPUT";

        private ScrapperContext _context = context;
